...
class pagination {
	...
	public function calculate_pages($total_rows, $rows_per_page, $page_num, $amountofblocks=5, $offset = 0) {
		$arr = array();
		// calculate last page
		...
		if (isset($_GET[$this->link])){
			$querystring = str_replace($this->link . "=" . $_GET[$this->link], "", $querystring);
		}
		...
		$pages = $this->get_surrounding_pages($page_num, $last_page, $arr['next'], $amountofblocks);
		if (!isset($pages[0]['p']))$pages = array();
